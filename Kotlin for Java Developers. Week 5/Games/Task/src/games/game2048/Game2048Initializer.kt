...
object RandomGame2048Initializer : Game2048Initializer<Int> {
    ...
    override fun nextValue(board: GameBoard<Int?>): Pair<Cell, Int>? {
        if (!board.any { it == null }) return null
        ...
        do {
            i = Random.nextInt(board.width) + 1
            j = Random.nextInt(board.width) + 1

            val cell = board.getCellOrNull(i, j) ?: continue
        ...
        return Cell(i, j) to generateRandomStartValue()
    }
}